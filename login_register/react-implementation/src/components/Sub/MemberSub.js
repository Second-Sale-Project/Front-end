...
export default function MemberSub(props) {
    ...
    const GetPlanMember = async () => {
        try {
            const result = await axios.post("http://localhost:3001/api/GetPlanMember", { uId });
    ...
    const { planId} = plan;
    const due_date = new Date(plan.due_date).toLocaleString('zh-TW', {timeZone: 'Asia/Taipei'});
    return (
        <React.Fragment>
            ...
            <Link to={{
                ...
                <div type="button" className="sub h150px verticalcenter bgcolorformembersub borderbold fontxxlarge fontbold fontcolorwhite">方案{planId}</div>
            </Link>
            <div className="w100per martb30px marl10per positionrelative">
                <span>訂閱方案</span>
                <span className="staycenter">方案{planId}</span><br />
                <span>到期日</span>
